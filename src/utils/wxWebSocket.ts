...
export class WxWebSocket {
  ...
  constructor(url: string, protocols?: string | string[]) {
    ...
    Promise.resolve(
      socketResult as unknown as Taro.SocketTask | Promise<Taro.SocketTask>
    )
      .then((socketTask) => {
        this.attachSocketTask(socketTask)
      })
      .catch((error) => {
        console.error('[WxWebSocket] 连接失败:', error)
        this.readyState = WxWebSocket.CLOSED
        if (this.onerror) {
          this.onerror({ type: 'error', message: error?.errMsg || String(error) })
        }
      })
  }

  private attachSocketTask(socketTask: Taro.SocketTask) {
